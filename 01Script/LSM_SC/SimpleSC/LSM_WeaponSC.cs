...
public class LSM_WeaponSC : MonoBehaviourPunCallbacks
{
    ...
    public IEnumerator SwordEffect(int code)
    {
        ...
        yield return new WaitForSeconds(code == 0 ? 0.65f:0.8f);
        pos1 = weaponPos.position;

        yield return new WaitForSeconds(code == 0? 0.15f:0.1f);
        ...
        if (n.y <= 0) { n = -n; }
        myParent.AttackEffect_B(dummypos, n, Vector3.Distance(pos1, dummypos));
